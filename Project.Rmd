...
for (filename in c("raw2017", "raw2018", "raw2019", "raw2020")) {
    ...
    assign(paste0("data", get_year(filename)), # output dataframe is data[year]
...
for (filename in c("data2017", "data2018", "data2019", "data2020")) {
   ...
   assign(paste0("origvars", get_year(filename)), # output dataframe is origvars[year]
          ...
          data.frame(varname = c(paste0(get_letter(filename), varname), "region"), varlabel =  c(varlabel, "Регион"))
          ) # region is separate - does not vary depending on the wave
...
for (filename in c("data2017", "data2018", "data2019", "data2020")) {
  ...
  assign(filename, 
          subset(get(filename), select=get(paste0("origvars", get_year(filename)))[["varname"]])
          )
  # ^^^ works just as e.g. data2019 <- subset(data2019, select = origvars2019[["varname"]])
  # only in a general way
  
  # More tidying up and replacing "ЗАТРУДНЯЮСЬ ОТВЕТИТЬ", "НЕТ ОТВЕТА", "ОТКАЗ ОТ ОТВЕТА" with NA
  assign(filename, get(filename) %>% remove_val_labels() %>% na_if(99999996) %>% na_if(99999997) %>% na_if(99999998) %>% na_if(99999999) %>% na.omit()
         ) # omitting NA

  
  # Converting to factor or numeric
  num_vec = paste0(get_letter(filename), c("_age", "j13.2", "j13", "j8", "j72.173", "_inwgt", "_hhwgt", "f14", "j10", "j60", "j5a", "j5b", "h7.2"))
  ...
  df = df %>% rename_with(.fn = ~c("origsm", "inwgt", "occup08","marst","diplom", "age", "h5", "sat_job", "sat_con", "sat_pay", "sat_prom", "jstat", "jcode08", "industry", "j6", "i4", "j8", "j8_1", "j13", "j13_2", "j26", "j29c_1", "j29c_2", "j22", "j31", "j32", "children", "m3", "id_h", "j10", "hhwgt", "c1", "f14", "j11", "j11_1", "j21_3", "j23", "j5a", "j5b", "j60", "h7_2")
  ...
  origdf$newname <- c("origsm", "inwgt", "occupation","marital","educ", "age", "male", "sat_job", "sat_con", "sat_pay", "sat_prom", "jstat", "jcode08", "industry", "supervisor", "nationality", "log_hours", "at_home", "est_size", "log_avgincome", "owner", "emp_trust", "col_trust", "job_search", "job_lose", "other_job", "children", "health", "id_h", "log_lastincome", "hhwgt", "renter", "f14", "j11", "contract", "hazardous", "govt", "tenure", "tenure", "log_totincome", "h7_2", "region")
